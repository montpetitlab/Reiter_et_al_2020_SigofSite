...
```{r 16hr}
info16hr <- read_csv("samples_ava.csv") %>%
  filter(!is.na(tank)) %>%
  filter(hr > 6) %>%
  filter(hr < 20) %>%
  filter(!is.na(micro_id)) %>%
...
colnames(v19) <- gsub("outputs\\/htseq_all_organisms\\/", "", colnames(v19))
colnames(v19) <- gsub("_readcounts\\.txt", "", colnames(v19))
v19 <- dplyr::select(v19, c(colnames(v19)[colnames(v19) %in% info16hr$id], gene))
v19 <- v19[rowSums(v19[-which(names(v19) %in% "gene")]) > 0, ] # remove rows that sum to zero
v19 <- v19 %>%
  mutate_at(vars(-gene), funs(./sum(.))) # divide by total number reads -- eg make relative
v19 <- pivot_longer(v19, cols = -gene, names_to = 'id', values_to = 'count')
v19 <- left_join(v19, info16hr, by = "id")
...
v17 <- dplyr::select(v17, c(colnames(v17)[colnames(v17) %in% info16hr$id], gene))
v17 <- v17[rowSums(v17[-which(names(v17) %in% "gene")]) > 0, ] # remove rows that sum to zero
v17 <- v17 %>%
  mutate_at(vars(-gene), funs(./sum(.))) # divide by total number reads -- eg make relative
v17 <- pivot_longer(v17, cols = -gene, names_to = 'id', values_to = 'count')
v17 <- left_join(v17, info16hr, by = "id")

sixteen_hr_counts <- rbind(v17, v19)

sixteen_hr_counts <- sixteen_hr_counts  %>%
  mutate(organism = gsub("_.*", "", gene)) %>%
  mutate(organism = gsub("KLTH0.*", "KLTH0", organism)) %>%
  mutate(organism = gsub("PEGC.*", "PEGC", organism)) %>%
  mutate(organism = gsub("MWSF.*", "MWSF", organism)) %>%
...
```{r dnainrna16hr}
rna_uvarum_16hr <- sixteen_hr_counts %>%
  filter(organism == "Hanseniaspora uvarum") %>%
  group_by(micro_id) %>%
  summarize(rna_h_uvarum = sum(count)) 
uvarum16hr <- left_join(rna_uvarum_16hr, asv_uvarum, by = c("micro_id"))


lm_uvarum16hr <- lm(uvarum16hr$`Hanseniaspora uvarum` ~ uvarum16hr$rna_h_uvarum)
# summary(lm_uvarum16hr)
lm_plt_uvarum16hr <- ggplot(uvarum16hr, aes(x = `Hanseniaspora uvarum`, y = rna_h_uvarum)) +
  geom_point() +
  labs(x = "Relative abundance in DNA", y = "Relative abundance in RNA") +
  theme_minimal()

rna_apul_16hr <- sixteen_hr_counts %>%
  filter(organism == "Aureobasidium pullulans") %>%
  group_by(micro_id) %>%
  summarize(rna_apul = sum(count)) 
...
apul16hr <- left_join(rna_apul_16hr, asv_apul, by = c("micro_id"))
lm_apul16hr <- lm(apul16hr$`Aureobasidium pullulans` ~ apul16hr$rna_apul)
summary(lm_apul16hr)
ggplot(apul16hr, aes(x = `Aureobasidium pullulans`, y = rna_apul)) +
  geom_point() +
  labs(x = "Relative abundance in DNA", y = "Relative abundance in RNA") +
  theme_minimal()
```

```{r pltuvarum_2_6_16, fig.height=2.75, fig.width = 7}

uvarum16hr <- uvarum16hr %>%
  mutate(hour = "16")
...
uvarum_early_ferm <- rbind(uvarum2hr, uvarum6hr, uvarum16hr)

uvarum_early_ferm$hour <- factor(uvarum_early_ferm$hour, levels = c("2", "6", "16"))
uvarum_early_ferm_plt <- ggplot(uvarum_early_ferm, aes(x = `Hanseniaspora uvarum`, y = rna_h_uvarum, color = hour)) +
  geom_point(alpha = 0.8, aes(shape = hour), size = 3) +
  labs(x = "Relative abundance in DNA", y = "Relative abundance in RNA") +
  theme_bw() + 
  theme(legend.position = "bottom",
        axis.title = element_text(size = 9),
...
ggarrange(uvarum_composition, uvarum_early_ferm_plt, labels = c(" ", " "), widths = c(2.2, 1))
```
...
anosim_results <- data.frame(data = c("bacteria", "bacteria", "bacteria", 
                             ...
                             variable = c("vintage", "region", "site", 
                                          "vintage", "region", "site", 
                                          "vintage", "region", "site"),
                             ANOSIM = c(ano_bac_year$statistic, ano_bac_ava$statistic, 
...
anosim_results$ANOSIM <- round(anosim_results$ANOSIM, digits = 2)
...
```{r adonis2}
bac_dada_dist <- dist(x = bac_dada, method = "euclidean")
adonis_bac <- adonis2(bac_dada_dist ~ year + ava + ava_id, data = info_micro,
                      permutations = 9999)
adonis_bac <- adonis_bac %>%
  rownames_to_column("var") %>%
  as.data.frame() %>%
  filter(var %in% c("year", "ava", "ava_id")) %>%
  filter(`Pr(>F)` < .05) %>%
  mutate(data = "bacteria") %>%
  select(data, variable = var, PERMANOVA = R2)

fun_dada_dist <- dist(x = fun_dada, method = "euclidean")
adonis_fun <- adonis2(fun_dada_dist ~ year + ava + ava_id, data = info_micro,
                      permutations = 9999)
adonis_fun <- adonis_fun %>%
  rownames_to_column("var") %>%
  as.data.frame() %>%
  filter(var %in% c("year", "ava", "ava_id")) %>%
  filter(`Pr(>F)` < .05) %>%
...
trans_dist <- dist(x = summarized_counts, method = "euclidean")
adonis_trans <- adonis2(trans_dist ~ year + ava + ava_id, data = info,
                      permutations = 9999)
adonis_trans <- adonis_trans %>%
  rownames_to_column("var") %>%
  as.data.frame() %>%
  filter(var %in% c("year", "ava", "ava_id")) %>%
  filter(`Pr(>F)` < .05) %>%
  mutate(data = "transcriptome") %>%
  select(data, variable = var, PERMANOVA = R2)

adonis2_results <- rbind(adonis_bac, adonis_fun, adonis_trans)
adonis2_results$PERMANOVA <- round(adonis2_results$PERMANOVA, digits = 2)
adonis2_results$variable <- gsub("year", "vintage", adonis2_results$variable)
adonis2_results$variable <- gsub("ava_id", "site", adonis2_results$variable)
adonis2_results$variable <- gsub("ava", "region", adonis2_results$variable)

multi_results <- left_join(anosim_results, adonis2_results)
ggtexttable(multi_results, rows = NULL, 
            theme = ttheme("light", base_size = 10, colnames.style = colnames_style(size = 9, fill = "white")))
```
...
info <- read_csv("samples_ava.csv") %>%
  ...
  filter(hr < 38 | hr > 46) %>%
  filter(hr <74 | hr >89) %>%
  select(id, ts_sum_id, year, tank, ava_id, brix, hr)
...
end_of_ferm_avg <- info %>%
  filter(hr > 80) %>%
  summarize(mean_brix = mean(brix), sd_brix = sd(brix))

sixteen_ferm_avg <- info %>%
  filter(hr < 20) %>%
  summarize(mean_brix = mean(brix), sd_brix = sd(brix))
...
sixtyfour_ferm_avg <- info %>%
  group_by(year) %>%
  filter(hr > 18) %>%
  filter(hr < 80) %>%
  summarize(mean_brix = mean(brix), sd_brix = sd(brix))

two_ferm_avg <- read_csv("samples_ava.csv") %>%
  filter(!is.na(tank)) %>%
  filter(hr == 2) %>%
  group_by(year) %>%
  summarize(mean_brix = mean(brix), sd_brix = sd(brix))

six_ferm_avg <- read_csv("samples_ava.csv") %>%
  filter(!is.na(tank)) %>%
  filter(hr == 6) %>%
  group_by(year) %>%
  summarize(mean_brix = mean(brix), sd_brix = sd(brix))
...
```{r reads_per_lib}
bac_dada_prefilter <- read_tsv("../pn_amp/2019/outputs/16s/dada2/ASV_counts.tsv") %>%
  as.data.frame() %>%                                          # read in DF
  column_to_rownames("X1") %>%
  colSums() %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  select(sample, prefilter_16s = ".") %>%
  filter(grepl("201[679]", sample))

bac_dada_postfilter <- read_tsv("../pn_amp/2019/outputs/16s/dada2/ASV_counts.tsv") %>%
  filter(!X1 %in% bac_tax_contam$X1) %>%                       # remove cholorplast and mitochondria
  as.data.frame() %>%                                          # read in DF
  column_to_rownames("X1") %>%
  colSums() %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  select(sample, postfilter_16s = ".") %>%
  filter(grepl("201[679]", sample))

# tmp <- left_join(bac_dada_prefilter, bac_dada_postfilter) %>%
#   mutate(percent_remaining = postfilter_16s/prefilter_16s) 
# 
# mean(tmp$prefilter)
# mean(tmp$postfilter)
# mean(tmp$percent_remaining)

fun_dada_sum <- read_tsv("../pn_amp/2019/outputs/ITS/dada2/ASV_counts.tsv") %>%
  as.data.frame() %>%                                          # read in DF
  column_to_rownames("X1") %>%
  colSums() %>%
  as.data.frame() %>%
...
amplicon_sum <- left_join(bac_dada_prefilter, bac_dada_postfilter)  %>%
  left_join(fun_dada_sum)
# rnaseq ------------------------------------------------------------------
...
colnames(v19) <- gsub("outputs\\/htseq_all_organisms\\/", "", colnames(v19))
colnames(v19) <- gsub("_readcounts\\.txt", "", colnames(v19))
v19 <- select(v19, c(colnames(v19)[colnames(v19) %in% info$id])) 
v19 <- colSums(v19) %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  select(sample, rna_seq = ".")

v17 <- read_tsv("rna_seq/v2017/outputs/counts_all_organisms/raw_counts.tsv") %>%
  filter(!gene %in% c("__no_feature", "__ambiguous", "__too_low_aQual", "__not_aligned", "__alignment_not_unique"))
colnames(v17) <- gsub("outputs\\/htseq_all_organisms\\/", "", colnames(v17))
colnames(v17) <- gsub("_readcounts\\.txt", "", colnames(v17))
v17 <- select(v17, c(colnames(v17)[colnames(v17) %in% info$id]))
v17 <- colSums(v17) %>%
  as.data.frame() %>%
  rownames_to_column("sample") %>%
  select(sample, rna_seq = ".")

rna_seq_reads <- rbind(v17, v19)
# write out -----------------------------

write.xlsx(amplicon_sum, file="supplementary_tables/table_S8_reads.xlsx", sheetName="amplicon_reads", row.names=FALSE)
write.xlsx(rna_seq_reads, file="supplementary_tables/table_S8_reads.xlsx", sheetName="rna_seq_reads", append=TRUE, row.names=FALSE)
