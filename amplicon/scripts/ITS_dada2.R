...
forward_reads <- sort(c(list.files(path = "outputs/ITS/trimmed_17", pattern="_R1_trimmed.fq", full.names = T),
                        list.files(path = "outputs/ITS/trimmed_19", pattern = "_R1_trimmed.fq", full.names = T)))
reverse_reads <- sort(c(list.files(path = "outputs/ITS/trimmed_17", pattern="_R2_trimmed.fq", full.names = T),
                        list.files(path = "outputs/ITS/trimmed_19", pattern = "_R2_trimmed.fq", full.names = T)))

# make sure forward and reverse reads are matched
stopifnot(all.equal(
  gsub("_R1_trimmed.fq", "", forward_reads),
  gsub("_R2_trimmed.fq", "", reverse_reads)
))

# make sample vector from file names
samples <- gsub("_R1_trimmed.fq", "", forward_reads)
samples <- gsub("outputs\\/ITS\\/trimmed_17\\/", "", samples)
samples <- gsub("outputs\\/ITS\\/trimmed_19\\/", "", samples)

# generate file names for filtered forward and reverse reads
filtered_forward_reads <- paste0("outputs/ITS/filtered/", samples, "_R1_filtered.fq.gz")
filtered_reverse_reads <- paste0("outputs/ITS/filtered/", samples, "_R2_filtered.fq.gz")
