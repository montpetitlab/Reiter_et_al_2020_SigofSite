...
summarized_counts <- summarized_counts %>%
  ungroup() %>%
  mutate(sample = paste(ava, year, tank, sep = "_")) %>%
  select(-year, -tank) 
...
summarized_counts <- pivot_wider(summarized_counts, id_cols = c(ava, sample), names_from = gene, values_from = value)

write_tsv(summarized_counts, "rna_seq/avas/summarized_counts_ava.tsv")
saveRDS(summarized_counts, "rna_seq/avas/summarized_counts_ava.RDS")
